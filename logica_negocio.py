...
async def agregar_item_bd(cart_id: str, product_id: str, name: str, quantity: int, precio_unitario: int):
    """
    Inserta o actualiza un producto dentro de un carrito específico en la BD.
    """
    conn = await get_db_connection()
    try:
        ...
        check_query = "SELECT item_id, quantity FROM cart_items WHERE cart_id = $1 AND product_id = $2"
